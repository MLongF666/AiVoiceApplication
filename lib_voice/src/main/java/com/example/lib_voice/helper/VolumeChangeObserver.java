...
public class VolumeChangeObserver {
    ...
    private static class VolumeBroadcastReceiver extends BroadcastReceiver {
        private final WeakReference<VolumeChangeObserver> mObserverWeakReference;
        ...
        public void onReceive(Context context, Intent intent) {
                    ...
                    && (intent.getIntExtra(EXTRA_VOLUME_STREAM_TYPE, -1) == AudioManager.STREAM_MUSIC))
            {
                ...
                Log.i("VolumeChangeObserver", "observer:" + observer);
                if (observer != null) {
                    VolumeChangeListener listener = observer.getVolumeChangeListener();
                    Log.i("VolumeChangeObserver", "listener:" + listener);
                    if (listener != null) {
                        int volume = observer.getCurrentMusicVolume();
                        Log.i("VolumeChangeObserver", "volume:" + volume);
                        if (volume >= 0) {
